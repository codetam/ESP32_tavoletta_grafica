...
const char* SIGNED_UP_HTML="<!DOCTYPE html> <html> <head> <meta name='viewport' content='width=device-width, initial-scale=1'> <style> * { box-sizing: border-box; } body { margin: 0; font-family: 'Roboto', sans-serif; background-color: #f5f5f5; } .logo { margin-right: auto; margin-left: auto; display: block; } .navbar { background-color: #f5f5f5; padding: 16px; display: flex; justify-content: space-between; align-items: center; } .navbar img { height: 75px; } .container { padding: 40px; background-color: #F7F7F7; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); border-radius: 4px; max-width: 400px; margin: 5px auto; text-align: center; } h2 { font-size: 24px; font-weight: 500; margin-bottom: 20px; } p { font-size: 16px; margin-bottom: 20px; } .message { font-size: 15px; font-weight: 500; margin-bottom: 20px; } .button { display: block; background-color: #1a73e8; color: white; padding: 12px; margin: 16px 0 8px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; font-weight: 500; letter-spacing: 0.03em; transition: background-color 0.2s ease; width: 100%; } .button:hover { background-color: #0c5dd1; } .divider { display: flex; align-items: center; text-align: center; margin: 20px 0; } .divider hr { flex-grow: 1; height: 1px; margin: 0; border: none; background-color: #ccc; } .divider .left { margin-right: 10px; } .divider .right { margin-left: 10px; } .divider span { font-size: 14px; font-weight: bold; color: #666; text-transform: uppercase; letter-spacing: 1px; font-style: italic; /* add italic style */ } a { text-decoration: none; } </style> </head> <body> <div class='navbar'> <img class='logo' src='https://i.ibb.co/yPBvMrD/logo-min.png'></div> <div class='container'> <h2>Benvenuto nel portale</h2> <p>Utente registrato con successo!</p> <p class='message'>Per visualizzare i disegni salvati vai su Area Personale. Se vuoi cambiare account vai su Cambia Account.</p> <a href='/profile' class='button'>Area Personale</a> <div class='divider'> <hr class='left'> <span>OR</span> <hr class='right'> </div> <a href='/' class='button'>Cambia Account</a> </div> </body> </html>";
...
const char* PROFILE_HTML="<!DOCTYPE html> <html> <head> <meta name='viewport' content='width=device-width, initial-scale=1'> <style> * { box-sizing: border-box; } body { margin: 0; font-family: 'Roboto', sans-serif; background-color: #f5f5f5; } .logo { display: block; margin: 0 auto; max-width: 100%; height: auto; } .navbar { background-color: #f5f5f5; padding: 16px; display: flex; justify-content: center; align-items: center; } .navbar img { height: 75px; } .container { padding: 40px; background-color: #F7F7F7; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); border-radius: 4px; max-width: 400px; margin: 0 auto; } .form-group { display: grid; grid-template-columns: 1fr; gap: 12px; } .form-group label { font-weight: bold; } .form-group input { padding: 12px; border-radius: 4px; border: 1px solid #ccc; } .numberButton { background-color: #BD161C; color: white; padding: 12px; margin-top: 16px; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; font-weight: 500; letter-spacing: 0.03em; transition: background-color 0.2s ease; width: 100%; } .numberButton:hover { background-color: #99181e; } @media screen and (max-width: 600px) { .container { padding: 20px; } } </style> </head> <body> <div class='navbar'> <img class='logo' src='https://i.ibb.co/yPBvMrD/logo-min.png'> </div> <div class='container'> <form action='/profile/number' method='post'> <h2>Inserisci un numero</h2> <p>Inserire il numero associato all'immagine da portare sulla tavoletta.</p> <div class='form-group'> <label for='numberInput'>Numero:</label> <input type='number' id='numberInput' name='number'> </div> <button type='submit' class='numberButton'>Inserisci</button> </form> </div> </body>";
...
void ConnectionHandler::createWebServer()
{
  ...
  server.on("/login", HTTP_POST, [this](AsyncWebServerRequest *request){
        ...
        if(uid != "" && pwd != "" && action != ""){
          int status = this->post_to_server("iot.dayngine.com", 8056, "/remote/login.php", "uid=" + uid + "&pwd=" + pwd + "&action=" + action, false);
          if(action == "login"){
            if(status == 200){
  ...
  server.on("/profile/number", HTTP_POST, [this](AsyncWebServerRequest *request){
        if (request->hasParam("number", true)) {
            imageId = request->getParam("number", true)->value();
        }
        if(dBusername != "" && dBpassword != "" && imageId != ""){
          int status = this->post_to_server("iot.dayngine.com", 8056, "/remote/load_image.php", "uid=" + dBusername + "&pwd=" + dBpassword + "&imageId=" + imageId, true);
            if(status == 200){
              request->send(200, "text/html", IMAGE_LOADED);
              imageId = "";
            }
            else{
              request->send(200, "text/html", IMAGE_NOT_LOADED);
            }
          }
        else{
          request->send(200, "text/html", IMAGE_NOT_LOADED);
...
int ConnectionHandler::upload()
{
  ...
  int status = post_to_server("iot.dayngine.com", 8056, "/remote/upload_image.php", post_data, false);
  return status;
}
